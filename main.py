...
def main():
    ...
    TARGET_DIR = os.path.join(OSINT_ROOT, "targets", args.target)
    ...
    try:
        ...
        for i, item_data in enumerate(posts_queue):
            ...
            try:
                ...
                # Parse soup once per post to avoid redundant parsing in actions
                soup = BeautifulSoup(driver.page_source, "html.parser")

                if not args.tagged:
                    if not action.verify_post_owner(driver, args.target, soup=soup):
                        log.debug(f"Skipping post (not owner)")
                        continue

                # Extract Metadata
                metadata = action.extract_metadata(driver, soup=soup)
                if 'likes' in item_data:
                ...
                if not downloaded_any:
                     ...
                     media_items = action.extract_media_from_post(driver, soup=soup)
